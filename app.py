...
def process_image():
    try:
        ...
        text = pytesseract.image_to_string(gray_image, lang='fra', config=tessdata_dir_config)
...
def process_oldcni():
    try:
        ...
        # Apply OCR using Tesseract
        text = pytesseract.image_to_string(new_img_gray, lang='fra', config=tessdata_dir_config)
        text = re.sub(r'[\u0600-\u06FF]', '', text)
